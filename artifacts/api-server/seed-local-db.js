...
// process.cwd() will be artifacts/api-server
...
const frontendEnvPath = path.join(process.cwd(), "..", "smartboss", ".env");
...
// Find and load the 'pg' package dynamically across monorepo directories
const searchPaths = [
  process.cwd(),
  path.join(process.cwd(), ".."),
  path.join(process.cwd(), "..", ".."),
  path.join(process.cwd(), "..", "..", "lib", "db"),
...
for (const p of searchPaths) {
  try {
    pgPath = require.resolve("pg", { paths: [p] });
    if (pgPath) {
      break;
    }
  } catch (e) {}
...
if (!pgPath) {
  console.error("Loyihada 'pg' moduli topilmadi! Iltimos, tekshiring.");
  process.exit(1);
}

console.log(`'pg' moduli muvaffaqiyatli aniqlandi: ${pgPath}`);
